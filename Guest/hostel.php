...
<body>
<form action="" method="post" enctype="multipart/form-data" name="form11" id="form11">
<?php
include 'db1.php';
include 'header.html';
if(isset($_POST['submit']))
{
	...
	move_uploaded_file($tmp1,"../Hostel/Photos/".$photo);
	$doc=$_FILES['doc']['name'];
	$tmp2=$_FILES['doc']['tmp_name'];
	move_uploaded_file($tmp2,"../Hostel/Documents/".$doc);
	$cph=mysql_query("select * from tb_hostel where mobile_no='$mobno'",$con);
	$cem=mysql_query("select * from tb_hostel where email_id='$email'",$con);
	...
	else
	{
		...
		{
			$a=$row['log_id'];
			$result=mysql_query("insert into tb_hostel (log_id,hostel_name,district,place,pin,Warden,No_of_rooms,mobile_no,email_id,picture,doc_upload,date) 
			values('$a','$name','$district','$place','$pin','$wname','$room','$mobno','$email','$photo','$doc','$date')",$con);
			if(isset($result)>0)
			{
...
<td><input required class="form-control" type="Text" name="wname" maxlength="20"></td>
</tr>
<tr>
<td><label><b>Number of rooms </b></label></td>
<td><input required class="form-control" type="Text" name="rooms"></td>
</tr>
<tr>
<td><label><b>Phone no </b></label></td>
<td><input required type="Text" class="form-control" name="mob" maxlength="10" pattern="[7-9]{1}[0-9]{9}" required></td>
</tr>
<tr>
...
<td><input required class="form-control" type="file" name="pic"></td>
</tr>
<tr>
<td><label><b>Upload Documents </b></label></td>
<td><input required class="form-control" type="file" name="doc"></td>
</tr>
<tr>
